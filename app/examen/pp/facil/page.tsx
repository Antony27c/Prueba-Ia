...
import { PP_FACIL_QUESTIONS } from '@/data/questions-pp-facil';

const ALL_QUESTIONS = PP_FACIL_QUESTIONS;
...
export default function PPFacilPage() {
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[#8b949e] text-xs">Nivel Fácil</span>
        </div>
  ...
  return (
    <div>
      ...
      <div className="w-full h-[6px] bg-[#21262d] rounded mb-3 overflow-hidden">
        <div className="h-full rounded transition-all duration-500" style={{ width: `${(answeredCount / TOTAL) * 100}%`, background: 'linear-gradient(90deg, #3fb950, #56d364)' }} />
      </div>
      ...
      <div className="flex gap-3">
        ...
        {currentIndex < TOTAL - 1 ? (
          ...
          <button onClick={submitExam} disabled={answeredCount < TOTAL}
            ...
            style={answeredCount < TOTAL ? {} : { background: 'linear-gradient(135deg, #3fb950, #56d364)' }}>
            Entregar examen
          </button>
        )}
      </div>
